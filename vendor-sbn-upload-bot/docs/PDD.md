...
## Process Overview
Once per day the bot extracts vendor master records created that day from SAP via an **SQVI query that joins LFA1 and ADRC** (so vendor email — held in the address tables — comes out alongside the master-data fields), maps six fields into the SBN-fixed CSV template, uploads that CSV to the SAP Business Network (SBN) web portal, polls the upload status until it resolves, and emails the team a summary with the CSV attached.
...
2. Log into SAP GUI.
3. Drive SAP GUI screen-by-screen with **UiPath SAP UI automation**: open **SQVI**, run the pre-built query (LFA1 ⋈ ADRC), enter **Create date = today**, execute, read the status bar for the no-records case, then export the result grid to a file (System → List → Export → Spreadsheet).
...
- SAP GUI (SQVI query joining LFA1 + ADRC) — source
- Microsoft Excel — CSV mapping/generation
- SAP Business Network (web) — upload target
- Microsoft Outlook — summary email

## Input / Output
- **Input:** Vendor records created today from the SQVI query (LFA1 ⋈ ADRC), exported from SAP GUI to a file.
...
Six fields, direct copy from the SQVI query output → SBN CSV: Vendor Name, Vendor ID, Tax ID, City, Country, Email (email sourced from ADRC via the join). Exact SBN header names/order taken from the user's SBN template file; exact query output column names taken from the built query.
...
**Native UiPath SAP UI automation, screen-by-screen** (revised from an earlier `.vbs`-scripting approach). **SAP GUI Scripting is enabled** in the environment — required for UiPath's SAP GUI selectors, which are built on the same scripting API. UiPath drives **SQVI** directly with UI activities (Type Into transaction code, run the pre-built LFA1⋈ADRC query, enter the create-date filter, execute, read the status bar for the empty case, export the grid to a file), then reads the exported file. Chosen for **maintainability**: all steps are visible and debuggable in Studio, no separate VBScript file and no `.vbs`↔UiPath result-file contract to maintain. Trade-off (a few more selectors, marginally slower) is negligible at this volume. All other work (mapping, web upload, status polling, email, error handling) stays in UiPath.

**Extraction changed from SE16N/LFA1 to an SQVI query** so vendor email (held in ADRC, not LFA1) is joined into the same result set.

## Prerequisites / Dependencies
- **SQVI query must be built by the business** — joining LFA1 ⋈ ADRC, outputting the six fields (Vendor Name, Vendor ID, Tax ID, City, Country, Email), with **create date as a selection parameter**.
