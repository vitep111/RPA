...
- [~] Phase 4 — Detailed Design (in progress)
  ...
  - **LESSONS LEARNED (PA traps):** `pa-desktop-reference.md` has a **Lessons Learned** section, **L1**, corrected after live user testing of Step 1.10b. Two wrong turns before landing on the verified answer: (1) first assumed `If` was single-condition only, so the initial multi-condition OR checks were wrong from the start; (2) "fixed" that by precomputing the OR into a Boolean via `Set variable` (e.g. `%A = "" OR B = ""%`) — **this also fails live**, `Set variable` errors comparing against an empty-string literal ("value cannot be empty"). **Verified correct (rule 4.4 ✅):** `If`'s own built-in condition list natively supports multiple conditions OR'd (or AND'd) together in one action — no flag needed. Step 1.10b and Step 3.10 both reverted to this direct multi-condition-`If` form; the `ConfigInvalid`/`BadHeaders` flag variables were removed. Reviewer now flags any `Set variable` that compares against an empty string (broken pattern, rule 1.7) or any manual splitting of an OR into stacked `If`s where the native multi-condition list would do it directly. Still open: whether `AND` and `Loop Condition` support the same multi-condition list (not yet tested live).
