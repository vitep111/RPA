...
- [~] Phase 4 — Detailed Design (in progress)
  ...
  - **LESSONS LEARNED (PA traps):** `pa-desktop-reference.md` now has a **Lessons Learned** section. **L1: PA Desktop `If` is single-condition** — no OR/AND-of-N in one `If` (unlike UiPath). Fix pattern: precompute the combined condition into one Boolean via `Set variable` (in-`%%` logical operators, rule 1.7), then `If %Flag% Equal to %True%`. Applied to Step 1.10b (`ConfigInvalid`) and Step 3.10 (`BadHeaders`). Reviewer now flags any `If`/`Loop Condition` needing more than one operand-operator-operand triple. Watch the same family in later phases: AND-combined `If`s, compound `Loop Condition` guards, inline ternaries (PA has none).
