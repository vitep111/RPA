...
- [~] Phase 4 — Detailed Design (in progress)
  ...
  - **LESSONS LEARNED (PA traps):** `pa-desktop-reference.md` has a **Lessons Learned** section, growing from live user testing:
    - **L1** (Step 1.10b): two wrong turns before the verified answer. (1) assumed `If` was single-condition only, so the initial multi-condition OR checks were wrong from the start; (2) "fixed" that by precomputing the OR into a Boolean via `Set variable` (e.g. `%A = "" OR B = ""%`) — **this also fails live**, `Set variable` errors comparing against an empty-string literal ("value cannot be empty"). **Verified correct (rule 4.4 ✅):** `If`'s own built-in condition list natively supports multiple conditions OR'd (or AND'd) together in one action — no flag needed. Step 1.10b and Step 3.10 both reverted to this direct multi-condition-`If` form; the `ConfigInvalid`/`BadHeaders` flag variables were removed.
    - **L2** (all 8 "Set Log Fields" call sites): `Set variable`'s Value field **cannot be left blank at all** — errors "parameter value can't be empty," even for a plain literal, not just an expression. The design's `LogUserID = (empty)` / `LogRequestID = (empty)` convention for run-level log rows (Fatal aborts, "No items") doesn't build. **Fix (rule 1.8 ✅):** every such call site now sets both fields to the literal `` `N/A` `` instead. Also caught and fixed the "Browser Fail" handler, which had omitted these fields entirely.
    - Reviewer now flags: any `Set variable` comparing to an empty string (L1/1.7), any `Set variable` left blank instead of using `N/A` (L2/1.8), and any manual OR-splitting into stacked `If`s where the native multi-condition list would do it directly (L1/4.4). Still open: whether `AND` and `Loop Condition` support the same multi-condition list (not yet tested live).
